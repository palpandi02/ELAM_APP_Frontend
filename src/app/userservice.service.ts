...
export class UserService{
  ...
  public loginUser(login: login): Observable<any> {
    ...
    return this.httpClient.post("http://localhost:9091/auth/authenticate", login, {
  ...
  public getUserRole(username: string): Observable<any> {
    const url = `http://localhost:9091/auth/getroles/${username}`;
  ...
  public getUserId(username: string): Observable<number> {
    const url = `http://localhost:9091/auth/getemployeeid/${username}`;
    return this.httpClient.get(url, { responseType: 'text' }).pipe(
      map(response => {

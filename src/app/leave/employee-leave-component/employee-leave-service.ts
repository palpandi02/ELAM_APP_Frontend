...
export class EmployeeLeaveComponentService {
  ...
  getLeaveBalance(employeeId: number): Observable<any[]> {
    ...
    const url = `${this.baseUrl}/balance/employee/${employeeId}`;
  ...
  applyLeave(leaveRequest: any): Observable<string> {
    const headers = this.getHeaders();
    return this.http.post(`${this.baseUrl}/leave/apply`, leaveRequest, { headers, responseType: 'text' }).pipe(
  ...
  getLeaveHistory(employeeId: number): Observable<any[]> {
    const headers = this.getHeaders();
    return this.http.get<any[]>(`${this.baseUrl}/leave/employee/${employeeId}`, { headers }).pipe(
      map(response => response),
      catchError(error => {
  ...
  deleteLeave(leaveId: number): Observable<string> {
    const headers = this.getHeaders();
    return this.http.delete(`${this.baseUrl}/leave/delete/${leaveId}`, { headers, responseType: 'text' }).pipe(
      map(response => response),
      catchError(error => {
